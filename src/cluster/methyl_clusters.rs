...
pub fn partition_cluster_set_by_exchange_groups(cluster_set: ClusterSet,
  ...
  -> Result<HashMap::<Vec::<usize>,ClusterSet>,CluEError>
{
  ...
  let mut n_cluster_partitions 
    = HashMap::<Vec::<usize>,Vec::<usize>>::with_capacity(n_groups);
  ...
  let mut cluster_partitions 
    = HashMap::<Vec::<usize>,Vec::<Vec<Cluster>>>::with_capacity(n_groups);
  ...
  let mut cluster_set_partitions 
    = HashMap::<Vec::<usize>,ClusterSet>::with_capacity(n_groups);
...
//------------------------------------------------------------------------------
// TODO: change key for Vec::<usize> to String.
fn get_cluster_partition_key(cluster: &Cluster,
    exchange_group_manager: &ExchangeGroupManager) -> Vec::<usize>
{
  let mut key = Vec::<usize>::with_capacity(cluster.len());
